...
const SemesterHostelBookingScreen = () => {
  ...
  const minPaymentAmount = Math.ceil(
    hostelDetails?.price || 0 * (minPaymentPercentage / 100)
  );
  ...
  const isPaymentValid = () => {
    return (
      ...
      paymentAmount <= hostelDetails?.price || 0
    );
  };
  ...
  const handleSubmit = () => {
    ...
    if (!isPaymentValid()) {
      Alert.alert(
        "Invalid Payment",
        `Payment amount must be between ${minPaymentAmount} and ${hostelDetails?.price || 0}`
      );
      return;
    }
    ...
    const bookingDetails = {
      ...
      balanceAmount: hostelDetails?.price || 0 - paymentAmount,
  ...
  const changeImage = (forward = true) => {
    if (forward) {
      setCurrentImageIndex(
        (currentImageIndex + 1) % hostelDetails.images.length
      );
    } else {
      setCurrentImageIndex(
        currentImageIndex === 0
          ? hostelDetails.images.length - 1
          : currentImageIndex - 1
      );
    }
  };
      ...
      <ScrollView className="flex-1 bg-purple-50 mb-4">
            ...
            <View>
              ...
              <View className="flex-row items-center mt-1">
                ...
                <Text className="text-white ml-1">
                  {hostelDetails?.location.address}
                </Text>
              </View>
            </View>
          ...
          <View className="bg-white rounded-2xl overflow-hidden shadow-md">
            <View className="h-56 bg-gray-200">
              ...
              <View className="w-full h-full items-center justify-center bg-indigo-100">
                <Image
                  source={{ uri: hostelDetails.images[currentImageIndex] }}
                  className="w-full h-full"
                />
                <Text className="text-indigo-600 mt-2">
                  Hostel Image {currentImageIndex + 1}
                </Text>
              </View>
        ...
        <View className="mx-4 mb-4">
          ...
          <View className="flex-row flex-wrap">
            {hostelDetails.amenities.map((amenity, index) => (
              <View
                key={index}
                className="bg-white rounded-full px-3 py-1 mr-2 mb-2 shadow-sm flex-row items-center"
              >
        ...
        <View className="px-4 pb-4">
          ...
          <View className="bg-teal-50 rounded-2xl p-4 shadow-sm mb-4 border border-teal-100">
            <View className="flex-row items-center mb-3">
              <Ionicons
                ...
                className="mr-2"
              />
              <Text className="text-teal-700 font-medium text-lg ml-2">
                Semester Details
              </Text>
            </View>
          ...
          <View className="bg-pink-50 rounded-2xl p-4 shadow-sm mb-4 border border-pink-100">
            <View className="flex-row items-center mb-3">
              <FontAwesome5
                ...
                className="mr-2"
              />
              <Text className="text-pink-700 font-medium text-lg ml-2">
                Payment Details
              </Text>
            </View>
            ...
            <View className="mb-4">
              <Text className="text-gray-600 mb-2">
                Payment Amount ({minPaymentPercentage}%
                required)
              </Text>
              ...
              <View className="h-2 w-full bg-gray-200 rounded-full overflow-hidden">
                <View
                  ...
                  style={{
                    width: `${Math.min(
                      100,
                      (paymentAmount / hostelDetails?.price || 0) * 100
                    )}%`,
                  }}
                />
              </View>
          ...
          <View className="bg-amber-50 rounded-2xl p-4 shadow-sm mb-4 border border-amber-100">
            <View className="flex-row items-center mb-3">
              <Ionicons
                ...
                className="mr-2"
              />
              <Text className="text-amber-700 font-medium text-lg ml-2">
                Special Requests
              </Text>
            </View>
          ...
          <View className="bg-indigo-50 rounded-2xl p-4 shadow-sm mb-4 border border-indigo-100">
            <View className="flex-row items-center mb-3">
              <Ionicons
                ...
                className="mr-2"
              />
              <Text className="text-indigo-700 font-medium text-lg ml-2">
                Hostel Rules
              </Text>
            </View>
          ...
          <TouchableOpacity
            ...
            style={{
              backgroundColor:
                acceptRules && isMobileValid() && isPaymentValid()
                  ? "#4f46e5"
                  : "#9ca3af",
            }}
